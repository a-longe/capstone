...
PieceInput = tuple[str, tuple[int, int]]
INPUT_GLYPH = 0
INPUT_COORDS = 1
...
def create_checkerboard(surface):
    ...
    for i in range(1, BOARD_SIZE + 1):
        for j in range(1, BOARD_SIZE + 1):
            if is_odd(i):
                if is_odd(j):
                    surface.blit(w_tile_img, BOARD_RECT[i - 1][j - 1])
                else:
                    surface.blit(b_tile_img, BOARD_RECT[i - 1][j - 1])
            elif not is_odd(i):
                if is_odd(j):
                    surface.blit(b_tile_img, BOARD_RECT[i - 1][j - 1])
                else:
...
class Piece:
    def __init__(self, board, glyph, coords, is_dragging) -> None:
        ...
        self.colour = 'w' if glyph.isupper() else 'b'
...
class Board:
    def __init__(self, input_pieces:list[PieceInput]) -> None:
        self.pieces = []
        for input_piece in input_pieces:
            self.pieces.append(Piece(self, input_piece[INPUT_GLYPH],
                                     input_piece[INPUT_COORDS], False))
    ...
    def reset_board(self) -> None:
        create_checkerboard(self.display_surface)
...
def game_event_loop(Board):
    for event in pg.event.get():
        if event.type == pg.MOUSEBUTTONDOWN:
            ...
            for player in Board.get_pieces():
                player.click = False
