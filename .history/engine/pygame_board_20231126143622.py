...
PieceDisplay = (pg.Surface, tuple[int, int])
...
def get_piece_surface(surface:pg.Surface, glyph:str, cords:tuple[int, int]) -> None:
    piece_img = get_piece_img(glyph)
    surface.blit(piece_img, cords)

def add_piece_to_buffer(buffer:list, piece_surface:pg.Surface, coords:tuple[int, int]):
    buffer.append((piece_surface, coords))
...
def reset_board(board:pg.Surface) -> None:
    board.blit(CHECKER_BASE, (0, 0))

def display_pieces(board:pg.Surface, buffer:list[tuple[pg.Surface, tuple[int, int]]]) -> None:
    reset_board(board)
    for piece_coord in buffer:
        piece_surface, coords = piece_coord
        board.blit(piece_surface, coords)
...
if __name__ == '__main__':
    ...
    piece_buffer = []
    add_piece_to_buffer(piece_buffer, get_piece_img('K'),
                        board_position_to_cords('a1'))
    add_piece_to_buffer(piece_buffer, get_piece_img('q'),
                        board_position_to_cords('b2'))
    while True:
        fps.tick()
        pg.display.update()
        print(pg.event.get(pg.K_SPACE))
        
        print(pg.mouse.get_pressed())
        if pg.mouse.get_pressed()[0] == True:
            print(piece_buffer)
            add_piece_to_buffer(piece_buffer, get_piece_img('q'),
                                board_position_to_cords('b3'))
        display_pieces(checkerboard, piece_buffer)
