...
def display_piece(surface:pg.Surface, glyph:str, cords:tuple[int, int]) -> None:
...
if __name__ == '__main__':
    ...
    checkerboard = pg.Surface((WINDOW_WIDTH, WINDOW_HEIGHT))
    checkerboard.blit(CHECKER_BASE, (0, 0))
    display_piece(checkerboard, 'K', board_position_to_cords('b7'))
    while True:
        if pg.mouse.get_pressed()[0]:
            print(get_mouse_pos())
            checkerboard.blit(CHECKER_BASE, (0, 0))
            display_piece(checkerboard, 'q', board_position_to_cords('b6'))
            display_piece(checkerboard, 'P', get_mouse_pos())
