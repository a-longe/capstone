...
from piece import get_piece_obj
...
class Board:
    ...
    def get_fen(self) -> str:
        ...
        for row_index in range(const.NUM_ROWS):
            ...
            fen_pieces += row_str
        fen_pieces = fen_pieces[:-1] # take away last char to remove last /
        fen += fen_pieces + ' '
        ...
        castling_rights_str = ''
        for castling_key in self.castling_rights.keys():
            if self.castling_rights[castling_key]:
                castling_rights_str += castling_key
    ...
    def get_all_moves(self, color:str) -> list[tuple[str, str]]:
        return []


    def get_board_after_move(self, move:tuple): # returns a board
        pass