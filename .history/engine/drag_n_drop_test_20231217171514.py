...
class Piece:
    ...
    def snap_to_square(self) -> None:
        if self.board.mouse_inside_bounds():
            ...
            old_index = self.board.get_square_index(*self.previous_center)
            new_index = self.board.get_square_index(*player_cords)
            print(f"({old_index}, {new_index})")
            self.board.piece_map[new_index] = deepcopy(self)
            del self.board.piece_map[old_index]
