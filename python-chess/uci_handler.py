...
def call_stockfish(commands: list[str], timeout_limit=1) -> str:
    cmd_string = "\n".join(commands)
    responce = subprocess.check_output(f'printf "{cmd_string}" | {STOCKFISH_RUN_COMMAND}'
                                            , shell=True, text=True, timeout=timeout_limit)
    return responce
...
def get_bestmove(fen:str, time_ms:int, moves:str) -> str:
    sf_responce = call_stockfish([f"position fen {fen} moves {moves}", f"go movetime {str(time_ms)}", "ucinewgame"], timeout_limit=int(time_ms * MS_TO_SEC_CONVERSION) + 1)
